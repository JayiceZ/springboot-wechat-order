...
public class ProductInfoServiceImpl implements ProductInfoService {
    ...
    @Override
    public List<ProductInfo> findAll(int start,int len) {
        return productInfoDao.findAll(start,len);
    }
    ...
    @Override
    public ProductInfo onSale(String product_id) {
        ProductInfo productInfo = productInfoDao.findOne(product_id);
        if (productInfo==null){
            throw new SellException(ResultEnum.PRODUCT_NOT_EXIST);
        }
        if (productInfo.getProduct_status()== ProductStatusEnum.UP.getCode()){
            throw new SellException(ResultEnum.PRODUCT_STATUS_ERROR);
        }
        //更新状态
        productInfo.setProduct_status(ProductStatusEnum.UP.getCode());
        productInfoDao.updateProductStatus(productInfo);
        return productInfo;
    }

    @Override
    public ProductInfo offSale(String product_id) {
        ProductInfo productInfo = productInfoDao.findOne(product_id);
        if (productInfo==null){
            throw new SellException(ResultEnum.PRODUCT_NOT_EXIST);
        }
        if (productInfo.getProduct_status()== ProductStatusEnum.DOWN.getCode()){
            throw new SellException(ResultEnum.PRODUCT_STATUS_ERROR);
        }
        //更新状态
        productInfo.setProduct_status(ProductStatusEnum.DOWN.getCode());
        productInfoDao.updateProductStatus(productInfo);
        return productInfo;
    }

    @Override
    public void updateProductAll(ProductInfo productInfo) {
        productInfoDao.updateProductAll(productInfo);
    }
}
